...
struct SegTreeBeatExtNode {
  ...
  using Self = SegTreeBeatExtNode<T>;
  Self *left;
  Self *right;
  T first_largest;
  T second_largest;
  ...
  void push_up() {
    ...
    first_largest_cnt =
        ...
        (right->first_largest == first_largest ? right->first_largest_cnt : 0);
  ...
  SegTreeBeatExtNode(int l, int r, const Indexer<T> &indexer): left(NULL),right(NULL) {
    if (l < r) {
      int m = (l + r) / 2;
...
private:
  using Node = SegTreeBeatExtNode<T>;
  Node *tree;
  int n;
...
#ifdef DROP
  ~SegTreeBeatExt() { Node::Destroy(tree); }
#endif
};
#undef ASSERTION_STUB
} // namespace segtree_beat_ext
using segtree_beat_ext::SegTreeBeatExt;
} // namespace dalt