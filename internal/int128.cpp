#pragma once
// credited to https://github.com/zhanhb/int128/blob/master/src/int128.h
...
#error endian not support
 ...
 public:
  ...
  constexpr explicit int128_base(double val_)
  ...
  constexpr int128_base(long val_)
      : int128_base(static_cast<long long>(val_)) {}  // NOLINT explicit

  constexpr int128_base(int val_)
      : int128_base(long(val_)) {}  // NOLINT explicit
  ...
  constexpr int128_base(unsigned val_)
      : int128_base(static_cast<unsigned long>(val_)) {}  // NOLINT explicit
 ...
 public:
  ...
  constexpr explicit operator long double() const {
    return cast_to_float<long double>();
  }
  ...
  friend constexpr int128_base operator+(int128_base lhs_, int128_base rhs_) {
    ...
    return {_Hi(lhs_.high_ + rhs_.high_ + (lhs_.low_ + rhs_.low_ < lhs_.low_)),
            lhs_.low_ + rhs_.low_};
  }
...
template <char... _Args>
constexpr int128_t operator"" _l128() {
...
struct clz_helper<unsigned long long> {
  static constexpr int clz(unsigned long long val_) {
...
struct detail_delegate {
  template <class _Hi, class _Low>
  static constexpr bool cmp(int128_base<_Hi, _Low> lhs_,
  ...
  template <class _Hi, class _Low>
  static constexpr int128_base<_Hi, _Low> imul(int128_base<_Hi, _Low> lhs_,
                                               int128_base<_Hi, _Low> rhs_) {
  ...
  template <class _Hi, class _Low>
  static constexpr int128_base<_Hi, _Low> shl(int128_base<_Hi, _Low> lhs_,
                                              unsigned rhs_) {
    // [64,127], 64 {low_ << 0, 0}
    return rhs_ & 64U
               ? int128_base<_Hi, _Low>(_Hi(lhs_.low_ << (rhs_ & 63U)), _Low(0))
  ...
  static uint128_t &slow_div_(uint128_t &dividend_, uint128_t divisor_,
                              uint128_t &quot_) {
  ...
  static int128_t div(int128_t dividend_, int128_t divisor_) {
    ...
    auto res_ = div(uint128_t(nneg_ ? -dividend_ : dividend_),
                    uint128_t(dneg_ ? -divisor_ : divisor_));
  ...
  static void part_div(uint128_t value_, uint64_t div_, uint64_t &high_,
                       uint64_t &mid_, uint64_t &low_) {
  ...
  template <class _Tp>
  constexpr static _Tp cast_to_float(uint128_t val_) {
    return std::ldexp(_Tp(val_.high_), 64) + _Tp(val_.low_);
  }

  template <class _Tp>
  constexpr static _Tp cast_to_float(int128_t val_) {
    return val_.high_ < 0 ? -cast_to_float<_Tp>(uint128_t(-val_))
                          : cast_to_float<_Tp>(uint128_t(val_));
  }
};
...
struct detail_delegate<true> {
  ...
  static constexpr ti_int_ to_native(int128_t val_) {
    return static_cast<ti_int_>(val_);
  }

  static constexpr tu_int_ to_native(uint128_t val_) {
    return static_cast<tu_int_>(val_);
  }

  static constexpr int128_t from_native(ti_int_ val_) { return int128_t(val_); }

  static constexpr uint128_t from_native(tu_int_ val_) {
  ...
  static constexpr int128_t sar(int128_t lhs_, unsigned rhs_) {
    return from_native(
        to_native(lhs_) >>
        static_cast<decltype(to_native(lhs_))>(rhs_));  // NOLINT signed shift
  }
  ...
  template <class _Hi, class _Low>
  static constexpr int128_base<_Hi, _Low> div(int128_base<_Hi, _Low> lhs_,
                                              int128_base<_Hi, _Low> rhs_) {
    return from_native(to_native(lhs_) / to_native(rhs_));
  }

  template <class _Hi, class _Low>
  static constexpr int128_base<_Hi, _Low> mod(int128_base<_Hi, _Low> lhs_,
                                              int128_base<_Hi, _Low> rhs_) {
    return from_native(to_native(lhs_) % to_native(rhs_));
  }
...
constexpr bool operator<(int128_t lhs_, int128_t rhs_) {
  return detail_delegate<>::cmp(lhs_, rhs_);
}

constexpr bool operator<(uint128_t lhs_, uint128_t rhs_) {
  return detail_delegate<>::cmp(lhs_, rhs_);
}
...
constexpr int128_t operator*(int128_t lhs_, int128_t rhs_) {
  return detail_delegate<>::imul(lhs_, rhs_);
}

constexpr uint128_t operator*(uint128_t lhs_, uint128_t rhs_) {
  return detail_delegate<>::imul(lhs_, rhs_);
}
...
inline uint128_t operator/(uint128_t lhs_, uint128_t rhs_) {
  return detail_delegate<>::div(lhs_, rhs_);
};

inline int128_t operator/(int128_t lhs_, int128_t rhs_) {
  return detail_delegate<>::div(lhs_, rhs_);
};
...
inline std::basic_ostream<_CharT, _Traits> &print_value(
    std::basic_ostream<_CharT, _Traits> &out_, bool signed_integral_,
    uint128_t value_) {
  ...
  switch (base_flag_) {
    ...
    default: {
      ...
      detail_delegate<>::part_div(value_, UINT64_C(10000000000000000000), high_,
                                  mid_, low_);
      if (high_) {
        offset_ =
            snprintf(buf_, buf_size_, "%" PRIu64 "%019" PRIu64 "%019" PRIu64,
                     high_, mid_, low_);
      } else if (mid_) {
        offset_ =
...
inline std::basic_ostream<_CharT, _Traits> &operator<<(
    std::basic_ostream<_CharT, _Traits> &out, uint128_t _Val) {
  return print_value(out, false, _Val);
}

template <class _CharT, class _Traits>
inline std::basic_ostream<_CharT, _Traits> &operator<<(
    std::basic_ostream<_CharT, _Traits> &out, int128_t _Val) {
...
#define MAKE_TYPE(outter, inner, parent)                     \
  template <>                                                \
  struct outter<large_int::inner> : std::parent {};          \
  template <>                                                \
  struct outter<const large_int::inner> : std::parent {};    \
  template <>                                                \
  struct outter<volatile large_int::inner> : std::parent {}; \
  template <>                                                \
  struct outter<const volatile large_int::inner> : std::parent {};
...
#define MAKE_TYPE(outter, inner, target)           \
  template <>                                      \
  struct outter<large_int::inner> {                \
    typedef large_int::target type;                \
  };                                               \
  template <>                                      \
  struct outter<const large_int::inner> {          \
    typedef const large_int::target type;          \
  };                                               \
  template <>                                      \
  struct outter<volatile large_int::inner> {       \
    typedef volatile large_int::target type;       \
  };                                               \
  template <>                                      \
  struct outter<const volatile large_int::inner> { \
